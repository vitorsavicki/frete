<?php 
	...
	switch ($GET) {
		...
		case 'verificar':
			...
			INNER JOIN tb_pessoa ON tb_pessoa.idPes = tb_transporte.tb_pessoa_idPes
			INNER JOIN tb_perfil ON tb_perfil.idPer = tb_pessoa.tb_perfil_idPer
			WHERE statusAleMen = 'N' AND tb_lance.tb_pessoa_idPes = $idPes AND tb_perfil.codigoPer = 'U'  AND  tb_mensagem.tb_pessoa_idPes <> $idPes";
		...
		case 'getnots' :
			...
			INNER JOIN tb_pessoa ON tb_pessoa.idPes = tb_transporte.tb_pessoa_idPes
			INNER JOIN tb_perfil ON tb_perfil.idPer = tb_pessoa.tb_perfil_idPer
			WHERE tb_lance.tb_pessoa_idPes = $idPes AND tb_perfil.codigoPer = 'U' AND  tb_mensagem.tb_pessoa_idPes <> $idPes
			ORDER BY dataRecebidaAleMen DESC, statusAleMen DESC LIMIT 4";
			$o_data = $o_db->query($st_queryAlerta);
			$o_rowsAlerta = $o_data->fetchAll(PDO::FETCH_OBJ);
			$total = count($o_rowsAlerta);
			...
			foreach($o_rowsAlerta as $o_ret => $value){
				  $li .='<li id="'.$value->idAleMen.'" style="background-color: white;">
                        <a href="?controle=Mensagem&acao=listarMensagemTransp&opcao=Mensagens" id="'.$value->idAleMen.'">
                          <div class="pull-left">
