<?php
	...
	//$_SESSION['idPes'] = 3;
	$idTransp = isset($_POST['tb_transporte_idTransp']) ? $_POST['tb_transporte_idTransp'] : '0';
	...
	$st_query .= "tb_avaliacao.valorAva5) / 5),1)) as media, ";
	...
	$st_query .= "WHERE TQ.Tb_transporte_idTransp ='$idTransp' ";
	$st_query .= "AND TQ.dataLan = (SELECT MAX(dataLan) FROM tb_lance WHERE tb_pessoa_idPes = TQ.tb_pessoa_idPes AND tb_lance.tb_transporte_idTransp = '$idTransp' GROUP BY tb_pessoa_idPes) ";
    ...
    $st_query .= "+ tb_avaliacao.valorAva3 ";
	$st_query .= "+ tb_avaliacao.valorAva4 ";
	$st_query .= "+ tb_avaliacao.valorAva5) / 5),1) ";
    ...
    $html .= '<thead>';
    $html .= '<tr>';
	$html .= '<th style="text-align:center;">Transportador</th>';
    $html .= '<th style="text-align:center;">Data do Lance</th>';
    $html .= '<th style="text-align:center;">Transportes Efetuados</th>';
    $html .= '<th style="text-align:center;">Avaliação</th>';
    $html .= '<th style="text-align:center;">Lance</th>';
	...
	foreach ($o_rows as $o_ret => $value)
	{
		...
		$html .= '<br>';
		$html .= '<p>';
		$html .= '<a href="?controle=Pessoa&acao=perfilTransportador&tb_pessoa_idPes='.$value->tb_pessoa_idPes.'&valorLance='.$value->valorLan.'&idLan='.$value->idLan.'&opcao1='.$opcao.'&opcao2=verLance&idTransp='.$value->tb_transporte_idTransp.'" class="btn btn btn-primary btn-xs">Ver Perfil</a>';
		$html .= '</p>';
		$html .= '</div>';
		$html .= '</div>';
    	$html .= '</td>';
    	$html .= '<td style="text-align:center;">';
    	$html .=  date('d/m/Y', strtotime($value->dataLan));
    	$html .= '</td>';
    	$html .= '<td style="text-align:center;">';
    	$html .=  $value->qtdeTransportes;
    	$html .= '</td>';
    	$html .= '<td style="text-align:center;">';
    	$html .= '<input id="valorAva1" name="valorAva1" type="number" class="rating" min="0" max="10" step="1" data-size="xs" data-symbol="&#xf005;" data-glyphicon="false" data-rating-class="rating-fa" class="required-entry form-required aria-required form-control" readonly="" value="'.$value->media.'" data-clearable="remove">';
		$html .= '</td>';
    	$html .= '<td style="text-align:center;">';
    	$html .=  number_format($value->valorLan, 2, ',', '.');
    	$html .=  '</td>';
    	//$html .=  '<td align="center">';
		$html .= '<td style="text-align:center;"><a data-toggle="modal" data-id="'.$value->idLan.'" title="Aceitar Lance" class="open-AddBookDialog btn btn btn-success btn-xs" href="#addBookDialog">Aceitar</a></td>';
   		...
   		$html .= '</td>';
    	//$html .= '</td> ';
    	$html .= '<td style="text-align:center;"><a type="button" id="packageDetails'.$value->idLan.'" name="packageDetails'.$value->idLan.'" class="btn btn-info btn-xs" class="accordion-toggle" data-toggle="collapse" data-parent="#OrderPackages" data-target=".packageDetails'.$value->idLan.'" onclick=fncJsonMensagem('.$value->idLan.');>Mensagens</a></td>';
		$html .=  '</tr>';
