<?php
    ...
    $st_query .= "      (select valorlan from tb_lance where tb_lance.tb_transporte_idTransp = tb_transporte.idTransp and vencedorLan = 'S') as vencedor";
	...
	elseif ($opcao == 'transpAndamentoTrans') { 
	$html .= '<th style="text-align:center;" >Meu Lance<br>Vencedor</th>';     
	}
    elseif ($opcao == 'transpandamento') { 
	...
	foreach ($o_rows as $o_ret => $value)
	{
		...
		$html .= '<td style="text-align:center;">'.'R$ ' .number_format($value->menorLan, 2, ',', '.').'</td>';
        if ($opcao == 'transpganhos') { 
		$html .= '<td style="text-align:center;">'.'R$ ' .number_format($value->vencedor, 2, ',', '.').'</td>';
        }
		...
		if ($opcao == 'clienteativos') 
		{
            ...
            $o_rows = $o_data->fetchAll(PDO::FETCH_OBJ);
            if($o_rows){
                $html .= '<td style="text-align:center;"><a class="btn btn-success btn-xs" href="?controle=Lance&acao=ListarLance&tb_transporte_idTransp='.$value->idTransp.'&opcao='.$opcao.'" class="btn btn-success btn-xs"  onclick="loadLance()">Ver Lances</a></td>';
            }
            else{
